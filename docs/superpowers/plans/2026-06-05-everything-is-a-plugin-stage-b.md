...
> ## ⚠️ DRAFT — NOT execution-ready. Rework required before implementing.
>
> This is a structurally sound first draft (good task decomposition, a complete
> workspace `BlockKind` audit incl. `ctrl/mod.rs`, the action-carrier decision, and a
> compiles-at-every-step ordering). **But it must not be handed to an implementer as-is.**
> Reviewed 2026-06-05 against real source; the following must be fixed first:
>
> 1. **Task 1 `apply_change_type` is a behavior-changing rewrite, not a faithful re-key.**
>    The real function (`actions.rs` ~521) is a `(target, source-body)` conversion *matrix*
>    that **preserves inline runs** where it can — e.g. Inline→List does
>    `List(vec![ListItem { runs: runs.clone() }])` (one item, formatting kept). The draft's
>    generic `body_to_flat_text` flattens everything to `InlineRun::plain(...)`, **losing
>    bold/italic** on conversions, splits Inline→List into multiple items, and invents an
>    unreachable Inline→Table coercion. Rewrite Task 1 to re-key the *existing* matrix arm
>    for arm onto editor-key/`body_shape`, changing only `BlockKind`→`PluginMeta`/attrs —
>    NOT to a generic flatten. The Opaque/Table no-op guards (present in the draft) are correct.
> 2. **Pervasive placeholders.** Tasks 2–9 contain `...`, `{ ... }`, `// ... existing X ...`,
>    "same as before" (e.g. lines ~548, 818, 906–932, 1000, 1159, 1313, 1523, 1661–1664,
>    1703–1840). Every one must be replaced with complete, real code before execution.
> 3. **Deliberation prose** left in Task 4 (~line 963, "Actually, let me reconsider…"). Strip it.
> 4. **Verification gap:** ChangeType + undo behavior is NOT covered by the round-trip suite.
>    Task 9 rewrites the action/undo tests — guard against re-baselining them to new (buggy)
>    behavior. Add explicit run-preservation assertions, and verify ChangeType/undo in the
>    real editor (the `driving-lopress-editor` control server) before claiming done.
>
> Recommended: harden this plan (faithful Task 1 + fill placeholders) and execute it
> **attended**, since it deletes the core type and its key behaviors can only be confirmed
> via the GUI, not the gate.
...
**Goal:** `ChangeType` carries `{ new_editor: Rc<str>, attrs: Box<serde_json::Map<String, Value>> }` instead of `new_kind: BlockKind`. `apply_change_type` looks up the target descriptor, swaps `PluginMeta`, and coerces the body. The inverse snapshots old editor + old attrs + old body (fixing the lossy-undo NOTE). Mirror-sync in `apply_edit_attrs` is deleted (attrs are the only copy now). `ChangeType` is boxed to keep the 40-byte guard.
...
Expected: FAIL (compilation error — `new_kind` field doesn't exist on the new enum variant; or `BlockKind::Heading` not found).

- [ ] **Step 3: Reshape `BlockAction::ChangeType`** — replace the variant and its size guard:
...
fn apply_change_type(
    ...
    let old_editor = block
        .plugin
        .as_ref()
        .and_then(|m| m.editor.clone())
        .unwrap_or_else(|| Rc::from("paragraph"));
    let old_attrs = block
        .plugin
        .as_ref()
        .map(|m| m.attrs.clone())
        .unwrap_or_default();
    let old_body = block.body.clone();

    // Look up the target descriptor to get body_shape + default attrs.
    let target = descriptor::descriptor_for(&new_editor);
    let target_shape = target.map(|d| d.body_shape);

    // Build the new PluginMeta: descriptor defaults merged with provided attrs.
    let new_meta = match target {
        Some(desc) => {
            let mut attrs = desc.default_block().plugin.as_ref().map(|m| m.attrs.clone()).unwrap_or_default();
            for (k, v) in new_attrs.iter() {
                attrs.insert(k.clone(), v.clone());
            }
            PluginMeta {
                block_type_name: Rc::from(desc.editor),
                attrs,
                attr_decls: Rc::from([]),
                builtin: desc.builtin,
                editor: Some(Rc::from(desc.editor)),
                native: desc.native.map(|n| Rc::from(n)),
            }
        }
        None => {
            // Unknown editor key: keep the block's existing PluginMeta but
            // update the editor field. This shouldn't happen in normal use.
            let mut meta = block.plugin.as_ref().cloned().unwrap_or_else(|| PluginMeta {
                block_type_name: new_editor.clone(),
                attrs: (*new_attrs).clone(),
                attr_decls: Rc::from([]),
                builtin: false,
                editor: Some(new_editor.clone()),
                native: None,
            });
            meta.attrs = (*new_attrs).clone();
            meta.editor = Some(new_editor.clone());
            meta
        }
    };

    // Coerce the body to the target shape.
    let new_body = match target_shape {
        Some(BodyShape::Inline) => {
            BlockBody::Inline(vec![InlineRun::plain(body_to_flat_text(&block.body))])
        }
        Some(BodyShape::Code) => BlockBody::Code(body_to_flat_text(&block.body)),
        Some(BodyShape::List) => BlockBody::List(
            body_to_flat_text(&block.body)
                .split('\n')
                .map(|line| ListItem {
                    id: BlockId::new(),
        ...
        Some(BodyShape::Table) => body_to_flat_text(&block.body)
            .lines()
            .map(|line| {
                let cells: Vec<TableCell> = line
                    ...
                    .map(|cell| TableCell {
                        id: BlockId::new(),
                        runs: vec![InlineRun::plain(cell.to_string())],
                    })
                    .collect();
                TableRow {
                    id: BlockId::new(),
            ...
            .into_iter()
            .map(|row| TableData {
                align: vec![Align::None; row.cells.len()],
                rows: vec![row],
            })
            .next()
            .map(|td| BlockBody::Table(td))
            .unwrap_or(block.body.clone()),
        Some(BodyShape::Opaque) => body.clone(),
        None => body.clone(),
    };

    // If body coercion would change the shape, apply it.
    if let Some(shape) = target_shape {
        match (shape, &new_body) {
...
- [ ] **Step 5: Delete `inline_plugin_meta`** — this function is no longer needed since `apply_change_type` now reads directly from the descriptor. Delete the function and its `#[allow(clippy::unreachable)]` guard.

- [ ] **Step 6: Delete the mirror-sync in `apply_edit_attrs`** — remove these lines (around line 202-219):
...
- [ ] **Step 7: Update the `apply` match arm** for `ChangeType`:
...
- [ ] **Step 8: Add imports** at the top of actions.rs — add `BodyShape`, `TableCell`, `TableRow`, `TableData` from `types`, and `descriptor` from `crate::model`:

```rust
use crate::model::descriptor;
use crate::model::types::{
    Align, BlockBody, BlockId, BodyShape, EditorBlock, EditorDoc, InlineRun, ListItem,
    PluginMeta, TableCell, TableData, TableRow,
};
```

- [ ] **Step 9: Run to verify they compile and pass**

Run: `cargo test -p lopress-editor change_type_to_heading_snapshots_and_restores_body apply_edit_attrs_no_longer_mirrors_into_kind`
Expected: PASS (both).

- [ ] **Step 10: Run the full test suite** to catch any `BlockKind` references that still use the old `ChangeType` shape:
...
**Before (the inline match on `kind`):**
```rust
...
```rust
            ...
            // The source block's editor key (from plugin) tells us what type it is.
...
fn coerce_body_to_editor(editor: &str, body: BlockBody) -> BlockBody {
    ...
    match (shape, &body) {
        ...
        (BodyShape::Table, _) => body, // Table coercion is a no-op for now.
...
**Files:**
- Modify: `crates/lopress-editor/src/model/descriptor.rs` (`MenuEntry` → `slash_label`/`toolbar_label`, `menu: &'static [MenuEntry]`, `slash_menu_items()`)
...
- [ ] **Step 3: Add projection helpers** to descriptor.rs:
...
pub fn block_toolbar_for(
    ...
    // ...
    let is_current = same_editor(&current_editor, &entry_editor);
    let is_inline = is_inline_editor(&current_editor);
    // ...
    on_action_for_btn(BlockAction::ChangeType {
        block_id,
...
- [ ] **Step 2: Rewrite `same_kind` → `same_editor`:**

```rust
/// True when two editor keys represent the same type.
/// For heading, level must also match.
fn same_editor(a: &str, b: &str) -> bool {
    if a == b {
        return true;
    }
    // Heading with different levels: "heading" == "heading" but H2 ≠ H3.
    // The toolbar buttons carry level-specific entries (H1..H6), so we
    // need to compare the actual editor + level. Since the toolbar only
    // shows "heading" entries for H1-H3 (slash) and H1-H6 (toolbar),
    // and each entry has its own default_block, we compare by checking
    // if both are heading and have the same level.
    //
    // Simplification: the toolbar buttons are unique by label, so we
    // compare the button label (which encodes level for headings).
    // "H1" == "H1", "H2" == "H2", etc.
    false // handled by comparing button labels, not editor keys
}
```

Actually, let me reconsider. The toolbar buttons already have unique labels ("P", "H1", "H2", ..., "Code", "UL", "OL"). The current button is determined by matching the button's label against what the block currently IS. For heading buttons, the label encodes the level. So we need to compare the button's default_block's editor + level against the block's editor + level.

Let me simplify: the button labels are unique, so we compare by checking if the button's `default_block()` produces the same editor + attrs as the current block.
...
- [ ] **Step 4: Update the button click handler** to emit `ChangeType { new_editor, attrs }`:

For each button, derive `new_editor` and `attrs` from the button's `default_block()`:
```rust
let block = (entry.default_block)();
let meta = block.plugin.as_ref().unwrap();
...
- [ ] **Step 5: Update the call site** — find where `block_toolbar_for` is called (in `wrap_block` in `mod.rs`) and pass the new arguments:
...
- [ ] **Step 6: Delete the `use crate::model::types::BlockKind` import** from toolbar.rs.

- [ ] **Step 7: Delete the old `same_kind` and `is_inline_kind` functions and their tests.**

- [ ] **Step 8: Write the projection-equals-current test** in toolbar.rs:

```rust
#[test]
fn toolbar_entries_match_expected_order() {
    // The descriptor-projected toolbar must reproduce today's button order.
...
Run: `cargo test -p lopress-editor toolbar_entries_match_expected_order is_inline_editor_paragraph_and_heading`
Expected: PASS.

- [ ] **Step 10: Run the full test suite** — fix any remaining `BlockKind` references in toolbar.
...
Also update the `lang` and `type_name` reads similarly.

- [ ] **Step 5: Update the tests** — replace all `CtrlBlockKind::...` with `CtrlChangeTarget { editor: ..., attrs: ... }`:
...
fn change_type_maps_each_kind() {
    ...
    let cases = [
        (CtrlChangeTarget { editor: "paragraph".into(), attrs: None }, "paragraph", serde_json::Map::new()),
        (CtrlChangeTarget { editor: "heading".into(), attrs: Some({ let mut m = serde_json::Map::new(); m.insert("level".into(), 2.into()); m }) }, "heading", { let mut m = serde_json::Map::new(); m.insert("level".into(), 2.into()); m }),
        (CtrlChangeTarget { editor: "code".into(), attrs: Some({ let mut m = serde_json::Map::new(); m.insert("lang".into(), "rust".into()); m }) }, "code", { let mut m = serde_json::Map::new(); m.insert("lang".into(), "rust".into()); m }),
        (CtrlChangeTarget { editor: "list".into(), attrs: Some({ let mut m = serde_json::Map::new(); m.insert("ordered".into(), true.into()); m }) }, "list", { let mut m = serde_json::Map::new(); m.insert("ordered".into(), true.into()); m }),
    ];
...
#[test]
fn change_type_heading_level_clamped() {
    let (doc, raw) = doc_one_paragraph();

    // level 9 → clamped to 6
    let ctrl = CtrlAction::ChangeType {
        block_id: raw,
        target: CtrlChangeTarget {
            editor: "heading".into(),
            attrs: Some({ let mut m = serde_json::Map::new(); m.insert("level".into(), 9.into()); m }),
        },
    };
    match ctrl.into_block_action(&doc).expect("known id translates") {
...
- [ ] **Step 6: Delete `use crate::model::types::BlockKind` import** from ctrl/mod.rs (or keep it if other parts still use it — check the file).
...
mod opaque_identity_meta_tests {
    ...
    fn opaque_block_carries_identity_meta() {
        // An opaque block must carry a PluginMeta with editor: None and
        // block_type_name set to the unknown type — so it survives to_core
        // through the opaque arm.
    ...
    fn opaque_identity_meta_round_trips_to_core() {
        // An opaque block's PluginMeta must round-trip through to_core's
        // opaque arm byte-identically.
        let b = EditorBlock::opaque("lopress:video".to_string(), serde_json::json!({ "src": "a.mp4" }));
        let core = crate::model::to_core::block_to_core(&b);
        assert_eq!(core.r#type, "lopress:video");
        // The opaque arm stashes the original JSON in the body.
...
- [ ] **Step 3: Update `from_core.rs`** — the `plugin_block_from_core` function still has `(BlockKind, BlockBody)` pairs in its match:

**Before:**
```rust
let (kind, body) = match editor {
    "heading" => (BlockKind::Heading(...), BlockBody::Inline(...)),
    "code" => (BlockKind::Code { ... }, BlockBody::Code(...)),
    "list" => (BlockKind::List { ... }, BlockBody::List(...)),
    _ => (BlockKind::Paragraph, BlockBody::Inline(...)),
};
EditorBlock { id: BlockId::new(), kind, body, plugin: Some(plugin) }
```
...
let body = match editor {
    "heading" => {
        let level = inner.and_then(|c| c.attrs.get("level").and_then(serde_json::Value::as_u64)).and_then(|n| u8::try_from(n).ok()).unwrap_or(1);
        let text = inner.and_then(|c| c.text.as_deref()).unwrap_or("");
        BlockBody::Inline(parse_inline(text))
    }
    "code" => {
        let lang = inner.and_then(|c| c.attrs.get("lang").and_then(serde_json::Value::as_str)).unwrap_or("");
...
EditorBlock { id: BlockId::new(), body, plugin: Some(plugin) }
```

Also update `native_block_from_core` — the `BlockKind::Image` arm:
...
Also update `native_table_from_core` and `native_list_from_core` similarly.
...
Also update `plugin_block_to_core` — remove the `(&b.kind, &b.body)` match and read from `PluginMeta`:
...
fn plugin_block_to_core(b: &EditorBlock, meta: &PluginMeta) -> Block {
    ...
    let inner = match &b.body {
        ...
        BlockBody::List(items) => {
            // ... existing list serialization ...
        }
        BlockBody::Table(data) => {
            // ... existing table serialization ...
        }
        BlockBody::Opaque(value) => {
            // Opaque body: serialize the stashed JSON.
...
pub fn block_view(block: &EditorBlock, dnd: DndState, env: &BlockEnv) -> AnyView {
    ...
    let body = match &block.body {
        BlockBody::Code(_) => {
            // Non-builtin block with Code body: render fallback.
...
fn wrap_block(
    ...
    // ... same as before, just remove `kind` parameter
}
```

- [ ] **Step 6: Update `plugin.rs` (render_body)** — remove the `BlockKind` fallback:
...
- **Production code (10 files):** `actions.rs`, `ctrl/mod.rs`, `from_core.rs`, `to_core.rs`, `types.rs`, `editor_registry.rs`, `inline_editor.rs`, `mod.rs` (blocks), `plugin.rs`, `pane_key.rs`, `inspector.rs`, `slash_menu.rs`, `toolbar.rs`
